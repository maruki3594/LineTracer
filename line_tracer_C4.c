...
int phase_1(int fd, int *pin)
{
    ...
    while (1)
    {
        while (1)
        {
            ...
            // error = (state[0] * 0.5 + state[1] * 0.3 + state[2] * 0.0 + state[3] * -0.3 + state[4] * -0.5) - set_point;
            error = (state[0] * 2.0 + state[1] * 1.0 + state[2] * 0.0 + state[3] * -1.0 + state[4] * -2.0) - set_point;
